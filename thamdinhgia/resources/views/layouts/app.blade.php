...
<head>
  ...
  <style>
    :root{ --green:#1e9b5a; --green-2:#137c47; --green-3:#e8f7ef; --text:#0c0f0e; --radius:16px; --shadow:0 10px 26px rgba(17,68,43,.10); --header-h:76px }
    ...
    /* header */
    .hdr{ position:sticky; top:0; z-index:1100; backdrop-filter:saturate(130%) blur(10px); background:rgba(255,255,255,.88); border-bottom:1px solid rgba(30,155,90,.10); transition:box-shadow .25s, background .25s }
    ...
    .brand-mark{ width:30px; height:30px; border-radius:10px; background:radial-gradient(120% 120% at 20% 15%, #3de793 0%, var(--green) 45%, #0f5d3a 100%); box-shadow:0 4px 10px rgba(30,155,90,.22), inset 0 1px 0 rgba(255,255,255,.35) }
    ...
    .hotline{ display:inline-flex; align-items:center; gap:.45rem; padding:.5rem .95rem; border-radius:999px; font-weight:800; color:var(--green); background:#fff; border:1.5px solid rgba(30,155,90,.35) }
    .hotline:hover{ color:#fff; background:var(--green); border-color:var(--green) }
    ...
    .navmob{ position:fixed; inset: calc(var(--header-h)) 0 0 0; background:rgba(255,255,255,.98); transform:translateY(-6%); opacity:0; pointer-events:none; transition:transform .25s, opacity .25s; border-top:1px solid #e0eee6 }
    ...
    /* hero */
    .hero-full{ position:relative; min-height: calc(100dvh - var(--header-h)); background:center/cover no-repeat fixed; color:#fff; text-align:center; display:flex; align-items:center; justify-content:center; text-shadow:0 2px 10px rgba(0,0,0,.4) }
    ...
    .scroll-cue{ position:absolute; left:50%; bottom:18px; transform:translateX(-50%); width:40px; height:40px; border-radius:999px; border:2px solid rgba(255,255,255,.9); display:grid; place-items:center; animation:cue 1.6s ease-in-out infinite }
    ...
    .service-card{ display:flex; align-items:flex-start; gap:14px; padding:22px; border-radius:18px; background:#fff; border:1px solid #e9f0ea; box-shadow: var(--shadow); text-decoration:none; color:inherit; transition: transform .25s, box-shadow .25s, border-color .25s }
    ...
    .footer .grid{ display:grid; gap:1rem; align-items:start } @media (min-width:768px){ .footer .grid{ grid-template-columns:1fr 1fr 1fr } }
    .footer .item{ display:flex; align-items:flex-start; gap:.75rem } .footer .small-note{ color:#5a6a60; font-size:.9rem }
    /* responsive fixes */
    @media (max-width:1200px){ .brand-tag{ display:none } .hotline{ display:none } }
    @media (max-width:991px){ .navdesk{ display:none!important } } @media (min-width:992px){ .navbtn{ display:none } }
    /* sticky sidebar for service-done */
    .sticky-aside{
      position: sticky;
      top: calc(var(--header-h) + 16px);
    }
    @media (max-width: 991px){
      .sticky-aside{ position: static; } /* tắt sticky trên mobile */
    }
    /* ===== featured services strip (đều 5 cột, cao bằng nhau) */
.services-grid{
...
.svc-card .icon-wrap{
  flex:0 0 auto; border-radius:999px; background:#f2fbf6; padding:12px; display:grid; place-items:center;
...
<header id="header" class="hdr">
  <div class="container py-2 d-flex align-items-center justify-content-between">
    <a href="/" class="brand-wrap">
      ...
      <span><div class="brand-name">VINAP</div><div class="brand-tag">Chuyên Nghiệp - Minh Bạch - Chính Xác - Khách Quan</div></span>
    </a>
  ...
  <div class="navmob" id="navmob" role="dialog" aria-modal="true" aria-label="Menu">
    <div class="container py-2">
      ...
      @endforeach
      <a href="tel:19001234"><strong>Hotline:</strong> 1900 1234</a>
    </div>
  </div>
</header>
...
<script>
  ...
  addEventListener('load', setHH); addEventListener('resize', setHH); setTimeout(setHH,250); visualViewport?.addEventListener?.('resize', setHH);
  ...
  const navToggle = document.getElementById('navToggle'), navmob = document.getElementById('navmob'), shade = document.getElementById('navshade');
  ...
  // ẩn cue khi rời hero + snap lần kéo đầu
  (function(){
    const cue = document.getElementById('scrollCue'), hero = document.querySelector('.hero-full'), target = document.querySelector('#info-start');
    if(!cue||!hero||!target) return;
    const snap = ()=> target.scrollIntoView({behavior:'smooth', block:'start'});
    cue.addEventListener('click', snap);
    const CUEIO = new IntersectionObserver(es=>{ es.forEach(e=>{ const v=e.isIntersecting; cue.style.opacity=v?1:0; cue.style.pointerEvents=v?'auto':'none'; }) },{threshold:0.1});
    CUEIO.observe(hero);
    let armed=true,startY=null; const arm=()=>armed=(scrollY<innerHeight*0.6);
  ...
  // fallback ảnh (nếu link bậy như httpsum.photos)
...
<script>
// Animation xuất hiện từng phần (fade-up + stagger)
...
.reveal {
  ...
  transition: opacity 0.9s cubic-bezier(.22,.8,.32,1), transform 0.9s cubic-bezier(.22,.8,.32,1);
}
.reveal-show {
  opacity: 1;
